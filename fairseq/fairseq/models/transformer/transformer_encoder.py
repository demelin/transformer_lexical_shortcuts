...
class ShortcutTransformerEncoder(FairseqEncoder):
    """
    Transformer encoder consisting of *cfg.encoder.layers* layers. Each layer
    is a :class:`TransformerEncoderLayer`.
    ...
    def build_encoder_layer(self, cfg):
        layer = transformer_layer.ShortcutTransformerEncoderLayer(
            cfg, return_fc=self.return_fc
        )
        checkpoint = cfg.checkpoint_activations
        if checkpoint:
    ...
    def forward_scriptable(
        ...
        # TODO: New additions here
        shortcut_embeddings = torch.clone(x)
        ...
        for layer in self.layers:
            lr = layer(
                x, shortcut_embeddings, encoder_padding_mask=encoder_padding_mask if has_pads else None
            )
