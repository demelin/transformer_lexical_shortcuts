    ...
    def __init__(self, config, source_vocab_size, target_vocab_size, name):
        ...
        with tf.name_scope('{:s}_inputs_and_variables'.format(self.name)), tf.device('/cpu:0'):
            ...
            if config.track_gate_values:
                enc_layers = \
                    ['encoder_layer_{:d}'.format(layer_id) for layer_id in range(1, config.num_encoder_layers + 1)]
                dec_layers = \
                ...
                for layer_name in dec_layers:
                    self.gate_tracker[layer_name] = dict()
                    self.gate_tracker[layer_name]['self_attention_lexical_gate_keys'] = list()
                    self.gate_tracker[layer_name]['self_attention_lexical_gate_values'] = list()
                    self.gate_tracker[layer_name]['cross_attention_lexical_gate_keys'] = list()
                    self.gate_tracker[layer_name]['cross_attention_lexical_gate_values'] = list()
        ...
        if self.config.track_gate_values:
            with tf.name_scope('gate_summaries'):
                ...
                for layer_name in self.gate_tracker.keys():
                    for gate_name in self.gate_tracker[layer_name].keys():
                        avg_gate_activations = tf.reduce_mean(self.gate_tracker[layer_name][gate_name])
                        self.gate_tracker[layer_name][gate_name] = list()
                        gate_summaries.append(tf.summary.scalar(name='{:s}_{:s}'.format(layer_name, gate_name),
                                                                tensor=avg_gate_activations))
...
class TransformerEncoder(object):
    ...
    def encode(self, source_ids, source_mask):
        ...
        with tf.variable_scope(self.name):
            ...
            state_cache = [enc_inputs]
            # Propagate through the stack
            enc_output = enc_inputs
            for layer_id in range(1, self.config.num_encoder_layers + 1):
                enc_output, _ = \
                    self.encoder_stack[layer_id]['self_attn'].forward(enc_output, state_cache, self_attn_mask)
                enc_output = self.encoder_stack[layer_id]['ffn'].forward(enc_output)
                # Cache layer-wise encoder representations
                state_cache.append(enc_output)

                # Update gate-tracker
                if len(self.gate_tracker.keys()) > 0:
                    self.gate_tracker['encoder_layer_{:d}'.format(layer_id)]['lexical_gate_keys'] = \
                        self.encoder_stack[layer_id]['self_attn'].key_gate
                    self.gate_tracker['encoder_layer_{:d}'.format(layer_id)]['lexical_gate_values'] = \
                        self.encoder_stack[layer_id]['self_attn'].value_gate

        return state_cache, cross_attn_mask
    ...
    def _get_initial_memories(self, batch_size, beam_size):
        ...
        for layer_id in range(1, self.config.num_decoder_layers + 1):
            initial_memories['layer_{:d}'.format(layer_id)] = \
                {'keys': tf.tile(tf.zeros([batch_size, 0, 1 * self.config.hidden_size]), [beam_size, 1, 1]),
                 'values': tf.tile(tf.zeros([batch_size, 0, 1 * self.config.hidden_size]), [beam_size, 1, 1])}
            ...
            for layer_id in range(1, self.config.num_encoder_layers + 1):
                ...
                with tf.variable_scope(layer_name):
                    ...
                    self_attn_block = ShortcutsAttentionBlock(self.config,
                                                              ...
                                                              training=self.training,
                                                              shortcut_type=self.config.shortcut_type)
                    cross_attn_block = ShortcutsAttentionBlock(self.config,
                                                               self.float_dtype,
    ...
    def decode_at_train(self, target_ids, enc_output, cross_attn_mask):
        ...
        def _decode_all(target_embeddings):
            ...
            for layer_id in range(1, self.config.num_decoder_layers + 1):
                ...
                # Update gate-tracker
                if len(self.gate_tracker.keys()) > 0:
                    self.gate_tracker['decoder_layer_{:d}'.format(layer_id)]['self_attention_lexical_gate_keys'] = \
                        self.decoder_stack[layer_id]['self_attn'].key_gate
                    self.gate_tracker['decoder_layer_{:d}'.format(layer_id)]['self_attention_lexical_gate_values'] = \
                        self.decoder_stack[layer_id]['self_attn'].value_gate
                    self.gate_tracker['decoder_layer_{:d}'.format(layer_id)]['cross_attention_lexical_gate_keys'] = \
                        self.decoder_stack[layer_id]['cross_attn'].key_gate
                    self.gate_tracker['decoder_layer_{:d}'.format(layer_id)]['cross_attention_lexical_gate_values'] = \
