...
class Transformer(object):
    ...
    def __init__(self, config, source_vocab_size, target_vocab_size, name):
        ...
        with tf.name_scope('{:s}_inputs_and_variables'.format(self.name)), tf.device('/cpu:0'):
            ...
            if config.track_gate_values:
                ...
                for layer_name in enc_layers + dec_layers:
    ...
    def _get_losses(self):
        ...
        with tf.name_scope('{:s}_loss'.format(self.name)):
            ...
            # Loss fixed to 0 to speed up classifier training
            masked_loss = 0.0
            sentence_loss = 0.0
            batch_loss = tf.reduce_mean(logits * 0.)
...
class TransformerEncoder(object):
    ...
    def __init__(self,
        ...
        # Track layer output for lexical probing
        self.layer_outputs = \
            {'layer_{:d}'.format(layer_id): list() for layer_id in range(self.config.num_encoder_layers + 1)}
    ...
    def encode(self, source_ids, source_mask):
        ...
        with tf.variable_scope(self.name):
            ...
            for layer_id in range(1, self.config.num_encoder_layers + 1):
                ...
                # Save layer outputs
                if layer_id == 1:
                    # Also save input embeddings
                    self.layer_outputs['layer_0'].append(enc_inputs)
                self.layer_outputs['layer_{:d}'.format(layer_id)].append(enc_output)
...
class TransformerDecoder(object):
    ...
    def __init__(self,
        ...
        # Track layer output for lexical probing
        self.layer_outputs = \
            {'layer_{:d}'.format(layer_id): list() for layer_id in range(self.config.num_decoder_layers + 1)}
    ...
    def decode_at_train(self, target_ids, enc_output, cross_attn_mask):
        ...
        def _decode_all(target_embeddings):
            ...
            for layer_id in range(1, self.config.num_decoder_layers + 1):
                ...
                dec_output, _ = \
                    self.decoder_stack[layer_id]['cross_attn'].forward(dec_output, enc_output[-1], cross_attn_mask)
                ...
                if layer_id == 1:
                    # Also save input embeddings
                    self.layer_outputs['layer_0'].append(dec_input)
                self.layer_outputs['layer_{:d}'.format(layer_id)].append(dec_output)
